(function () {
  ...
  function initAdminCharts() {
    ...
    const chartFont = styles.getPropertyValue('--sq-font-body').trim() || 'Poppins, system-ui, sans-serif';
  ...
  async function refreshAdminDashboard() {
    ...
    const isInteracting = () => document.querySelector('.modal.show, .offcanvas.show')
      || current?.querySelector('details[open]')
    ...
    try {
      ...
      showAdminUpdateFailure();
    } finally {
      adminRefreshInFlight = false;
    }
  }

  function showAdminUpdateFailure() {
    const status = document.querySelector('[data-admin-refresh-status]');
    if (!status || status.classList.contains('is-stale')) return;
    status.textContent = `Updates unavailable; retrying automatically. ${status.textContent}`;
  ...
  function initAdminLiveDashboard() {
    ...
    adminQueueUnsubscribe = subscribe('', (payload, error, metadata = {}) => {
      if (error || !payload?.success) { showAdminUpdateFailure(); return; }
      if (metadata.source === 'initial') return;
