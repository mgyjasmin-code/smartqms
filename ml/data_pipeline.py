...
def load_dataset(
    ...
    """Load and validate database-exported SmartQMS queue observations."""
    ...
    missing = [column for column in REQUIRED_COLS if column not in source.columns]
    if missing:
        raise DatasetValidationError(
            "queue_data.csv must be generated by scripts/export_ml_history.php; "
            "missing columns: " + ", ".join(missing)
        )
    model_frame = source.loc[:, REQUIRED_COLS]
    return validate_dataset(model_frame, minimum_rows=required_rows)
