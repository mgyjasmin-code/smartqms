    ...
    <div class="container public-form-container">
      ...
      <?php else: ?>
        <section class="public-tracker" data-public-ticket-tracker data-token="<?= htmlspecialchars($token, ENT_QUOTES) ?>" data-status-url="<?= APP_URL ?>/modules/queue/public_ticket_status.php" data-feedback-url="<?= APP_URL ?>/modules/feedback/public_submit.php">
          <header class="public-workflow-heading public-tracker-heading"><p class="public-kicker"><?= $isScheduled ? 'Scheduled reservation' : 'Private live tracker' ?></p><div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-end gap-3"><div><h1><?= $isScheduled ? 'Your reservation is scheduled' : 'Follow your place in line' ?></h1><p data-service-name><?= htmlspecialchars($projection['service_name']) ?></p></div><button class="btn btn-outline-primary" type="button" data-enable-sound aria-pressed="false"><i data-lucide="bell" aria-hidden="true"></i> Enable alerts</button></div></header>
          ...
          <div class="public-tracker-grid" data-ticket-metadata>
            <article class="card public-ticket-card"><div class="card-body p-4 p-md-5"><div class="public-ticket-number-block"><span>Queue number</span><strong data-ticket-number data-queue-number><?= htmlspecialchars($projection['ticket_number'] ?: 'Assigned at check-in') ?></strong></div><dl class="public-ticket-details mt-4"><div><dt>Reference</dt><dd><?= htmlspecialchars($projection['reference_number']) ?></dd></div><div><dt>Visit date</dt><dd><?= htmlspecialchars(date('F j, Y', strtotime((string) $projection['visit_date']))) ?></dd></div><div><dt>Status</dt><dd data-status-detail><?= $isScheduled ? 'Scheduled' : htmlspecialchars(ucwords(str_replace('-', ' ', $projection['status']))) ?></dd></div><div><dt>People ahead</dt><dd data-people-ahead><?= $isScheduled ? 'Available after check-in' : number_format($projection['people_ahead']) ?></dd></div><div><dt>Estimated wait</dt><dd data-wait-display><?= $isScheduled ? 'Calculated at check-in' : '<span data-wait-minutes>' . number_format($projection['predicted_wait_minutes'], 1) . '</span> min' ?></dd></div><div><dt>Counter</dt><dd data-counter-label><?= htmlspecialchars($projection['counter_label'] ?: 'To be assigned') ?></dd></div><?php if ($isScheduled): ?><div><dt>Check-in period</dt><dd>8:00 AM–3:30 PM</dd></div><?php endif; ?></dl></div></article>
            <article class="card public-ticket-qr-card"><div class="card-body p-4 text-center"><h2 class="h5">Private tracker QR</h2><p class="text-body-secondary"><?= $isScheduled ? 'Present this code when you physically arrive.' : 'Scan to reopen this tracker on another device.' ?></p><?php if (!empty($ticket['qr_code_path'])): ?><img src="<?= APP_URL ?>/<?= htmlspecialchars($ticket['qr_code_path'], ENT_QUOTES) ?>" alt="QR code for this ticket's private tracker" width="220" height="220"><?php endif; ?></div></article>
          </div>
