(() => {
  ...
  const updateError = root.querySelector('[data-update-error]');
  let timer = 0;
  let inFlight = false;
  ...
  const statusCopy = {
    scheduled: ['Visit reserved', 'Present your QR code or reference to staff on your visit date.'],
  ...
  const render = (data, received = true) => {
    ...
    if (waitDisplay) {
      const rawWait = data.predicted_wait_minutes;
      const numeric = typeof rawWait === 'number' || (typeof rawWait === 'string' && /^\d+(\.\d+)?$/.test(rawWait.trim()));
      const valid = numeric && Number.isFinite(Number(rawWait)) && Number(rawWait) >= 0;
      const text = status === 'scheduled' ? 'After check-in'
        : (valid ? `${Number(rawWait).toFixed(1)} min` : 'Temporarily unavailable');
      if (waitDisplay.textContent !== text) waitDisplay.textContent = text;
    }
    ...
    if (received && lastUpdated) {
      const now = new Date();
      lastUpdated.dateTime = now.toISOString();
      const updatedText = `Last updated ${new Intl.DateTimeFormat(undefined, { hour: 'numeric', minute: '2-digit' }).format(now)}`;
      if (lastUpdated.textContent !== updatedText) lastUpdated.textContent = updatedText;
    }
  ...
  const poll = async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.success || !payload.data) throw new Error('Queue update unavailable');
      render(payload.data);
      if (updateError) updateError.hidden = true;
    } catch (_) {
      if (updateError) updateError.hidden = false;
    } finally {
      inFlight = false;
      schedule();
    }
  ...
  render({
    ...
    predicted_wait_minutes: root.querySelector('[data-wait-minutes]')?.textContent ?? null,
