...
Version: 3.3 (healthcare redesign, representative-screen review)  
...
Production CSS uses `--sq-*` variables and maps Bootstrap 5.3 component variables to that semantic layer. Legacy `--admin-*` variables are compatibility aliases or admin-only composition values; new color, typography, border, focus, radius, and shadow decisions must use `--sq-*` tokens.

Public, authentication, client, and staff surfaces load Bootstrap plus `style.css`. Admin adds `admin.css`; the public queue board adds `display.css`. Bootstrap, its JavaScript bundle, and licenses are Composer-managed, published locally, and hash-verified.
...
| Warning | `#9A6700` | `#FFD166` on `#493915` |
| Danger | `#B42318` | `#FF8A80` on `#4A252B` |
...
- Cards use white, blue-white, or the equivalent dark semantic surface, a 1px border, and no shadow.
